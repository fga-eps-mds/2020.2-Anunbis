...
class TestProfessorDetail(TestFlaskBase):
    ...
    def test_api_must_require_token(self):
        ...
        self.assertEqual(
            response.json, {'msg': 'Missing Authorization Header'})
...
class TestProfessorIdDatail(TestFlaskBase):
    def get(self, id, headers):
        return self.client.get(url_for('restapi.professoriddetail', id=id), headers=headers)
    ...
    def test_api_must_return_student_information_when_not_anonymous(self):
        ...
        student_expected = {'course': {
            'id_course': 1, 'name': self.course['name']}, 'name': 'Testing Student'}
